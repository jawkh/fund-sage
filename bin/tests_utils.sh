...
rpt_fileName="logs/utils-pytest-${timestamp}-RPT.log"
rpt_errFileName="logs/utils-pytest-${timestamp}-ERROR.log"
...
"$(which poetry)" run pytest --cov=utils tests/utils_tests 2> "$rpt_errorLogFilePath" | tee -a "$rpt_logFilePath"
